    ...
    try {
      ...
      if (!error && Array.isArray(data)) {
        return data.map((r) => ({
          ...
          title: r.title,
          prize: r.prize,
          prizesList: Array.isArray(r.prizes_list) ? r.prizes_list : (r.prize ? [r.prize] : []),
          prizesCount: r.prizes_count || 3,
          ticketPrice: r.ticket_price || '$2.500 ARS',
          sponsorName: r.sponsor_name || 'ON MÁS Portal Regional',
          imageUrl: r.image_url || '/images/city-federacion.jpg',
          drawDate: r.draw_date,
          status: r.status,
          autoActiveCommerces: r.auto_active_commerces ?? true,
